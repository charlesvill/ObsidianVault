...
```
- this is an example of implementing multer local disk storage. will store to the project directory /uploads file

#### Cloudinary and uploading files on the cloud
- this is an example of using cloudinary to upload a file that was piped from the multer memory storage which takes from stream buffer
```js
...
```
##### passing payload with Curl
ssssssotice here the use of the -X flag that is for the request to change the request type. 
`curl -X POST -H "Content-Type:application/json" http://localhost:3000/messages -d '{"text":"Hi again, World"}'`
- notice here more use of the curl -h flag can set the headers and -d flag is the data payload 
- make sure that you have the json middleware enabled `app.use(express.json()`
