...
-[ ] what does it mean to be hoisted? 
 ...
 - what are polyfills and polymorphism
 - 
...
3. dependency inversion
	- should abstract away lower level functtionality when looking at a higher level method. for example higher level method of processing payment, if successful do this, else, do that. this should not include the writing of how the payment is actually processed, this should be abstracted away in a seperate class so if you need to change who does payments, you can focus on the class instead of the higher leven functions that depend on the lower level stuff. it may look like more code, but it means better written code that will save you time refactoring down theline. 
		- revisiting Depedency inversion: good resource on DI https://www.geeksforgeeks.org/dependecy-inversion-principle-solid/?ref=header_search
		- in simpler terms its designing functions, classes, objects, etc in such a way that does not hard program the implementation of what it can do. that is, if its a higer level function. for example, a manager class that works on managing employees but has functions that implement adding very specific types of employees is tightly coupled to those employee functions and if you try adding a diffrent kind of employee, you need to refactor the code. instead, adding an interface like say passing employees as arguments allows you to accept all number of different kinds and they pulll from a general employee class that allows their responsibilities to also be defined in a modular way. 
